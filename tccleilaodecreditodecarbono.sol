...
contract LeilaoDeCreditoDeCarbono {
    ...
    uint256 public taxaLeiloeiro;
    
    uint quantidadeDisponivelEmToneladasDoLote;

    address payable public contaLeiloeiro;
    bool statusPagamentoTaxaLeiloeiro;
    ...
    constructor(
        ...
        address payable _contaLeiloeiro,
        string memory legislacaoAplicavel
        
    ) public {
        contaBovespa = _contaBovespa;
        contaLeiloeiro = _contaLeiloeiro;
        prazoFinalLeilao = now + _duracaoLeilao;
        quantidadeDisponivelEmToneladasDoLote = 500;
        leiDoContrato = legislacaoAplicavel;
        cotacaoAtualDoCreditoDeCarbonoEmEther = 16;
        incrementoMinimo = 1;
        comissaoDoLeiloeiro =5;
    }
    function lote01 () public view returns (uint) {
        return quantidadeDisponivelEmToneladasDoLote;
    }
    
    function balancoDoContrato() public view returns(uint256) {
    ...
    function finalizaLeilao() public somenteBovespa payable {
       
        require(now >= prazoFinalLeilao, "Leilao ainda nao encerrado.");
        require(!encerrado, "Leilao encerrado.");
        require(statusPagamentoTaxaLeiloeiro = true, "Taxa leiloeiro pendente");
        ...
        contaLeiloeiro.transfer(msg.value);
        
        statusPagamentoTaxaLeiloeiro = true;
    
    }
    
    function pagamentoTaxaLeiloeiro () public somenteBovespa payable {
        contaBovespa = msg.sender;
        taxaLeiloeiro = msg.value;
}
}
